...
<div class="admin-content">
    ...
    <div class="card" style="background-color: var(--color-white); border: 1px solid var(--color-gray-200); border-radius: var(--radius-lg); padding: var(--spacing-4); box-shadow: var(--shadow-sm);">
        <h1 style="margin-top: 0; margin-bottom: var(--spacing-4); color: var(--color-gray-900);">Edit Subject: {{ $subject->name }}</h1>
        ...
        <form method="POST" action="{{ route('admin.subjects.update', $subject) }}" style="display: grid; gap: var(--spacing-4);">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $subject->name) }}" required style="width: 100%; padding: var(--spacing-2); border: 1px solid var(--color-gray-300); border-radius: var(--radius-lg); font-family: var(--font-secondary); font-size: var(--font-size-base); box-sizing: border-box;">
            </div>
            ...
            <div>
                ...
                <textarea name="description" style="width: 100%; padding: var(--spacing-2); border: 1px solid var(--color-gray-300); border-radius: var(--radius-lg); font-family: var(--font-secondary); font-size: var(--font-size-base); box-sizing: border-box; min-height: 100px;">{{ old('description', $subject->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <input type="color" name="color" value="{{ old('color', $subject->color) }}" required style="width: 100%; padding: var(--spacing-2); border: 1px solid var(--color-gray-300); border-radius: var(--radius-lg); cursor: pointer; height: 40px;">
            </div>
            ...
            <div>
                ...
                <input type="number" name="order_by" value="{{ old('order_by', $subject->order_by) }}" required style="width: 100%; padding: var(--spacing-2); border: 1px solid var(--color-gray-300); border-radius: var(--radius-lg); font-family: var(--font-secondary); font-size: var(--font-size-base); box-sizing: border-box;">
            </div>

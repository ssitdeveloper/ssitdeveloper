...
<div class="admin-content">
    ...
    <div class="card" style="background-color: var(--color-white); border: 1px solid var(--color-gray-200); border-radius: var(--radius-lg); padding: var(--spacing-4); box-shadow: var(--shadow-sm);">
        <h1 style="margin-top: 0; margin-bottom: var(--spacing-4); color: var(--color-gray-900);">Create New Question</h1>
        ...
        <form method="POST" action="{{ route('admin.questions.store') }}" style="display: grid; gap: var(--spacing-4);">
            @csrf
            ...
            <div>
                ...
                <select name="chapter_id" required style="width: 100%; padding: var(--spacing-2); border: 1px solid var(--color-gray-300); border-radius: var(--radius-lg); font-family: var(--font-secondary); font-size: var(--font-size-base); box-sizing: border-box;">
                    ...
                    @foreach($chapters as $chapter)
                        <option value="{{ $chapter->id }}" @if(old('chapter_id') === (string)$chapter->id) selected @endif>
            ...
            <div>
                ...
                <textarea name="question_text" required style="width: 100%; padding: var(--spacing-2); border: 1px solid var(--color-gray-300); border-radius: var(--radius-lg); font-family: var(--font-secondary); font-size: var(--font-size-base); box-sizing: border-box; min-height: 120px;">{{ old('question_text') }}</textarea>
            </div>
            ...
            <div>
                ...
                <select name="difficulty_level" required style="width: 100%; padding: var(--spacing-2); border: 1px solid var(--color-gray-300); border-radius: var(--radius-lg); font-family: var(--font-secondary); font-size: var(--font-size-base); box-sizing: border-box;">
                    ...
                    @foreach($difficulties as $difficulty)
                        <option value="{{ $difficulty->value }}" @if(old('difficulty_level') === $difficulty->value) selected @endif>{{ ucfirst($difficulty->value) }}</option>
                    @endforeach
                </select>
            </div>

            <!-- Options -->
            <fieldset style="border: 1px solid var(--color-gray-300); border-radius: var(--radius-lg); padding: var(--spacing-3);">
                <legend style="padding: 0 var(--spacing-2); font-weight: var(--font-weight-semibold); color: var(--color-gray-900);">Options <span style="color: var(--color-danger);">*</span></legend>

                <div id="options-container" style="display: grid; gap: var(--spacing-3);">
                    <!-- Options will be dynamically added -->
                </div>

                <button type="button" onclick="addOption()" style="margin-top: var(--spacing-3); padding: var(--spacing-2) var(--spacing-4); background-color: var(--color-gray-200); color: var(--color-gray-900); border: none; border-radius: var(--radius-lg); font-weight: var(--font-weight-semibold); cursor: pointer;">
                    + Add Option
                </button>
            </fieldset>
            ...
            <div>
                ...
                <textarea name="explanation" style="width: 100%; padding: var(--spacing-2); border: 1px solid var(--color-gray-300); border-radius: var(--radius-lg); font-family: var(--font-secondary); font-size: var(--font-size-base); box-sizing: border-box; min-height: 80px; resize: vertical;">{{ old('explanation') }}</textarea>
            ...
            <div style="display: flex; align-items: center; gap: var(--spacing-2); padding: var(--spacing-3); background-color: var(--color-gray-50); border-radius: var(--radius-lg); border: 1px solid var(--color-gray-300);">
                <input type="checkbox" id="is_published" name="is_published" value="1" @checked(old('is_published', true)) style="width: 20px; height: 20px; cursor: pointer;">
                ...
                <span style="color: var(--color-gray-600); font-size: var(--font-size-sm);">Published questions will be visible to students</span>
            </div>
...
function addOption() {
    ...
    optionDiv.style.gridTemplateColumns = '1fr auto auto';
    optionDiv.style.gap = 'var(--spacing-2)';
    optionDiv.style.alignItems = 'center';

    optionDiv.innerHTML = `
        <input type="text" name="options[${index}][text]" placeholder="Option text" required style="padding: var(--spacing-2); border: 1px solid var(--color-gray-300); border-radius: var(--radius-lg); font-family: var(--font-secondary); font-size: var(--font-size-base); box-sizing: border-box;">
        <label style="display: flex; align-items: center; gap: var(--spacing-1); margin: 0; font-weight: var(--font-weight-medium); color: var(--color-gray-900); cursor: pointer;">
            <input type="radio" name="correct_option" value="${index}" style="width: 18px; height: 18px; cursor: pointer;">
            Correct
        </label>
        <button type="button" onclick="this.parentElement.remove()" style="padding: var(--spacing-1) var(--spacing-2); background-color: var(--color-danger); color: var(--color-white); border: none; border-radius: var(--radius-lg); cursor: pointer;">
            Remove
        </button>
    `;
